    ...
    <main className="min-h-screen bg-[#09090B] text-[#F0F0EC]" style={{ fontFamily: "var(--font-sans)" }}>
      <div className="mx-auto max-w-4xl px-5 py-8 sm:px-6">
        ...
        <header className="mt-8">
          <p className="text-xs font-black uppercase tracking-[0.16em] text-[#CAFF3C]">{fr ? "Visibilité IA" : "AI visibility"}</p>
          ...
          <p className="mt-4 max-w-2xl text-[#D7D7CD]">{vis.headline}</p>
        </header>
        ...
        <section className="mt-10 rounded-[1.4rem] border border-white/[0.08] bg-[#111116] p-6">
          <div className="flex flex-wrap items-end justify-between gap-2">
            ...
            <span className="text-sm text-[#B8B8C4]">{fr ? `Rang ${vis.brandRank}/${vis.playerCount} · ${vis.totalPrompts} questions` : `Rank ${vis.brandRank}/${vis.playerCount} · ${vis.totalPrompts} questions`}</span>
          </div>
          <ul className="m-0 mt-5 flex list-none flex-col gap-2.5 p-0">
            {barRows.map((player) => {
              const tone = player.isBrand ? "#CAFF3C" : "#5A6B58";
              ...
              return (
                <li key={`${player.isBrand ? "brand" : "c"}-${player.name}`} className="grid grid-cols-[9rem_1fr_2.5rem] items-center gap-3">
                  <span className={`truncate text-sm ${player.isBrand ? "font-black text-[#CAFF3C]" : "text-[#D7D7CD]"}`}>
                    {player.isBrand ? `${player.name} ${fr ? "(toi)" : "(you)"}` : player.name}
                  </span>
                  <span className="h-3 rounded-full bg-white/[0.06]">
                    <span className="block h-3 rounded-full" style={{ width: `${width}%`, backgroundColor: tone }} />
                  </span>
                  <span className="text-right text-sm tabular-nums text-[#B8B8C4]">{player.citedCount}/{vis.totalPrompts}</span>
                </li>
              );
            })}
          </ul>
        ...
        <section className="mt-6 rounded-[1.4rem] border border-white/[0.08] bg-[#111116] p-6">
          ...
          <p className="mt-2 text-sm text-[#B8B8C4]">{fr ? "Qui l'IA cite, question par question." : "Who AI cites, question by question."}</p>
          <ul className="m-0 mt-5 flex list-none flex-col gap-2 p-0">
            {vis.byPrompt.map((row) => (
              <li key={row.prompt} className="rounded-xl border border-white/[0.06] bg-white/[0.02] px-4 py-3">
                <div className="flex items-start justify-between gap-3">
                  <span className="text-sm leading-6 text-[#D7D7CD]">{row.prompt}</span>
                  <span className="whitespace-nowrap rounded-full px-3 py-1 text-xs font-black" style={{ backgroundColor: row.brandCited ? "#CAFF3C22" : "#77778622", color: row.brandCited ? "#CAFF3C" : "#777786" }}>
                ...
                {row.competitorsCited.length > 0 && (
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {row.competitorsCited.slice(0, 5).map((name) => (
                      <span key={name} className="rounded-full border border-white/10 px-2.5 py-0.5 text-xs text-[#B8B8C4]">{name}</span>
                    ))}
                  </div>
                )}
              </li>
        ...
        <section className="mt-6 rounded-[1.4rem] border border-white/[0.08] bg-[#111116] p-6">
          ...
          <p className="mt-2 text-sm text-[#B8B8C4]">{proof.headline}</p>
