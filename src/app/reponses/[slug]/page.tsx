    ...
    <main className="min-h-screen bg-[#F5F7FA] text-[#132A43]" style={{ fontFamily: "var(--font-sans)" }}>
      ...
      <section className="relative overflow-hidden border-b border-[#E4E9F0]">
        ...
        <div className="relative mx-auto max-w-4xl px-5 py-6 sm:px-6 sm:py-8">
          ...
          <div className="py-14 sm:py-18">
            <p className="mb-4 text-xs font-black uppercase tracking-[0.16em] text-[#123E5C]">{copy.eyebrow}</p>
            <h1 className="max-w-3xl text-[clamp(2.2rem,6vw,4.4rem)] leading-[0.95] tracking-[-0.05em]" style={{ fontFamily: "var(--font-display)" }}>
              {copy.title}
            </h1>
            <p className="mt-7 max-w-2xl text-lg leading-8 text-[#5B6B82]">{copy.directAnswer}</p>
          </div>
        </div>
      </section>
        ...
        <section className="mx-auto max-w-4xl border-b border-[#E4E9F0] px-5 py-10 sm:px-6 sm:py-12">
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            {assets.competitors.slice(0, 8).map((competitor) => (
              <span key={competitor} className="rounded-full border border-[#E4E9F0] px-3 py-1.5 text-sm text-[#5B6B82]">{competitor}</span>
            ))}
          </div>
        </section>
      )}
      ...
      <section className="mx-auto max-w-4xl px-5 py-12 sm:px-6 sm:py-16">
        <div className="grid gap-3">
          {copy.faq.map((item) => (
            <details key={item.question} open className="rounded-2xl border border-[#E4E9F0] bg-[#FFFFFF] p-5">
